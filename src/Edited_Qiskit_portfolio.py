...
# NOTICE!
# This file has been altered from its original code. The original file is available at :
# https://qiskit.org/documentation/_modules/qiskit/finance/applications/ising/portfolio.html
# Changes were made to adjust for a set of portfolios instead of investing in stocks directly.
...
# EDIT : Added portfolioMatrix and added 0 as a default penalty
def get_operator(mu, sigma, q,portfolioMatrix,budget,penalty=0):  # pylint: disable=invalid-name
    """ get qubit op """
    # pylint: disable=invalid-name

    # EDIT : Transform stock moments to portfolio moments
    mu = np.matmul(portfolioMatrix.T,mu)
    sigma = np.matmul(sigma,portfolioMatrix)
    sigma = np.matmul(portfolioMatrix.T,sigma)
...
# EDIT : Added portfolioMatrix and added 0 as a default penalty
def portfolio_value(x, mu, sigma, q,portfolioMatrix, budget, penalty=0):  # pylint: disable=invalid-name
...
def portfolio_expected_value(x, mu,portfolioMatrix,):  # pylint: disable=invalid-name
...
def portfolio_variance(x, sigma,portfolioMatrix):
